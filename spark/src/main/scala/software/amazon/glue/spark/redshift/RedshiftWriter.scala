...
import Parameters.{MergedParameters, PARAM_COPY_DELAY}
import software.amazon.glue.spark.redshift.data.{RedshiftConnection, RedshiftWrapper}
 ...
 */
private[redshift] class RedshiftWriter(redshiftWrapper: RedshiftWrapper) {
  ...
  private def copySql(
    ...
    copySqlToTable(sqlContext, schema, params, manifestUrl, params.table.get)
  }
  ...
  private def copySqlToTable(
    ...
    val credsString: String =
      AWSCredentialsUtils.getRedshiftCredentialsString(params)
  ...
  private def doRedshiftLoad(
    ...
    manifestUrl.foreach { manifestUrl =>
      ...
      val copyStatement = copySql(data.sqlContext, data.schema, params, manifestUrl)
  ...
  private def doRedshiftDelete(
    ...
    manifestUrl.foreach { manifestUrl =>
      // Load the temporary data into the new file
      val copyStatement = copySqlToTable(data.sqlContext, data.schema, params,
    ...
    Utils.retry(params.copyRetryCount, params.copyDelay) {
      ...
      try {
        ...
        if (params.isDelete) {
          ...
          doRedshiftDelete(conn, data, params, manifestUrl)
        } else {
          log.info("Loading new Redshift data to: {}", table)
          doRedshiftLoad(conn, data, params, manifestUrl)
        }
...
object DefaultRedshiftWriter extends
  RedshiftWriter(new RedshiftWrapper())
