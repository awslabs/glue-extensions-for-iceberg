...
import software.amazon.glue.spark.redshift.pushdown.{BooleanVariable, ByteVariable, ConstantString, DoubleVariable, FloatVariable, IntVariable, LongVariable, RedshiftSQLStatement, ShortVariable, StatementElement, StringVariable}
...
private[redshift] class DataApiWrapper extends RedshiftWrapper with Serializable {
    ...
    try {
      ...
      try {
        ...
        case e: SQLException =>
          ...
          throw new SQLException(s"Exception thrown in awaitResult: $errorMessagePart", e)
        case NonFatal(t) =>
          // Wrap and re-throw so that this thread's stacktrace appears to the user.
          throw new Exception(s"Exception thrown in awaitResult: $errorMessagePart", t)
      }
    } catch {
  ...
  private def executeBatch(conn: RedshiftConnection,
    ...
    val dataApiConnection = conn.asInstanceOf[DataAPIConnection]
    if (dataApiConnection.params.debug) {
      executeInterruptibly(new DataApiCommand(dataApiConnection), _.executeBatch(sqls), sqls.mkString(";"))
    } else {
  ...
  override def executeInterruptibly(conn: RedshiftConnection, sql: String): Boolean = {
    ...
    if (!conn.getAutoCommit()) {
      ...
      if (dataApiConnection.params.debug) {
        executeInterruptibly(new DataApiCommand(dataApiConnection), _.execute(sql), sql)
      } else {
  ...
  : RedshiftResults = {
    ...
    val dataApiConnection = conn.asInstanceOf[DataAPIConnection]
    if (dataApiConnection.params.debug) {
      executeInterruptibly(new DataApiCommand(dataApiConnection), _.executeQueryInterruptibly(sql), sql)
    } else {
  ...
  : Long = {
    ...
    if (!conn.getAutoCommit()) {
      ...
      if (dataApiConnection.params.debug) {
        executeInterruptibly(new DataApiCommand(dataApiConnection), _.executeUpdate(sql), sql)
      } else {
    ...
    Try {
      ...
      if (dataAPIConnection.params.debug) {
        val results = executeInterruptibly(
          new DataApiCommand(dataAPIConnection),
          _.executeQueryInterruptibly(sql), sql).asInstanceOf[DataApiResults].results
  ...
  private def getCatalystType(
    ...
    val answer = redshiftType match {
      ...
      case "timestamp without time zone" => TimestampNTZType
  ...
  private def tableMetaDataFromStatement(conn: RedshiftConnection,
    ...
    if (dataApiConnection.params.debug) {
      executeInterruptibly(
        new DataApiCommand(dataApiConnection, params), _.executeQueryInterruptibly(query), query)
    } else {
      executeInterruptibly(
