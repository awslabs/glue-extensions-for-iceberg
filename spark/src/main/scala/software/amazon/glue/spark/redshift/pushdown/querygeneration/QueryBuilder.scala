...
import software.amazon.glue.spark.redshift.{RedshiftFailMessage, RedshiftPushdownException, RedshiftPushdownUnsupportedException, RedshiftRelation}
import software.amazon.glue.spark.redshift.pushdown.{RedshiftDMLExec, RedshiftPlan, RedshiftSQLStatement, RedshiftScanExec, RedshiftStrategy}
import software.amazon.glue.spark.redshift.pushdown.optimizers.LeftSemiAntiJoinOptimizations.{isDistinctAggregate, isPassThroughProjection, isSetOperation, pullUpLeftSemiJoinOverProjectAndInnerJoin}
 ...
 */
private[redshift] class QueryBuilder(plan: LogicalPlan) {
  ...
  lazy val treeRoot: RedshiftQuery = {
    try {
...
private[redshift] object QueryBuilder {
  ...
  def getSparkPlanFromLogicalPlan(plan: LogicalPlan):
