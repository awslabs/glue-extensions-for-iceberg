...
import software.amazon.glue.spark.redshift.Parameters.MergedParameters
import software.amazon.glue.spark.redshift.Utils
import software.amazon.glue.spark.redshift.pushdown.{BooleanVariable, ByteVariable, ConstantString, DoubleVariable, FloatVariable, Identifier, IntVariable, LongVariable, RedshiftSQLStatement, ShortVariable, StatementElement, StringVariable}
...
private[redshift] class JDBCWrapper extends RedshiftWrapper with Serializable {
  ...
  override def resolveTable(
    ...
    try {
      ...
      val fields = {
        new scala.Array[StructField](ncols)
      }
      var i = 0
      while (i < ncols) {
  ...
  private def resolveTableFromMeta(rsmd: ResultSetMetaData,
    ...
    val ncols = rsmd.getColumnCount
    val fields = new scala.Array[StructField](ncols)
    var i = 0
    while (i < ncols) {
      val columnName = rsmd.getColumnLabel(i + 1)
      val rsType = rsmd.getColumnTypeName(i + 1)
  ...
  private def prepareWithBindVariable(conn: RedshiftConnection,
                                      sqlStatement: RedshiftSQLStatement): PreparedStatement = {
    val sql = sqlStatement.list.reverse
    val varArray: scala.Array[StatementElement] =
      new scala.Array[StatementElement](sqlStatement.numOfVar)
