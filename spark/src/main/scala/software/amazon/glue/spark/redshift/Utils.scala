...
private[redshift] object Utils {
  ...
  def assertThatFileSystemIsNotS3BlockFileSystem(uri: URI, hadoopConfig: Configuration): Unit = {
    ...
    if (fs.getClass.getCanonicalName == "software.amazon.glue.fs.s3.S3FileSystem") {
  ...
  def getDefaultRegion(): String = {
    ...
    if (currRegion != null) currRegion else Regions.US_EAST_1.getName
  }
  ...
  def handleRedshiftErrorResponse(errorMessage: String): Exception = {
    errorMessage match {
      case s: String if s.contains("permission denied for relation") ||
      s.contains("Insufficient Lake Formation permission") =>
        new AccessDeniedException(s)
      case _ => new RuntimeException(errorMessage)
    }
  }
}
