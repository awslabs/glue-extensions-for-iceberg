...
import Conversions.parquetDataTypeConvert
import Parameters.{MergedParameters, PARAM_OVERRIDE_NULLABLE}
import RedshiftRelation.schemaTypesMatch
...
import software.amazon.glue.spark.redshift.pushdown.{RedshiftSQLStatement, SqlToS3TempCache}
...
private[redshift] case class RedshiftRelation(
  ...
  override def insert(data: DataFrame, overwrite: Boolean): Unit = {
    ...
    val writer = new RedshiftWriter(redshiftWrapper)
  ...
  override def buildScan(requiredColumns: Array[String], filters: Array[Filter]): RDD[Row] = {
    ...
    if (requiredColumns.isEmpty) {
      ...
      val unloadSql = buildUnloadStmt(
        requiredColumns, filters, tempDir, params.sseKmsKey)
      val conn = redshiftWrapper.getConnectorWithQueryGroup(params, queryGroup)
      try {
  ...
  def buildScanFromSQL[Row](statement: RedshiftSQLStatement,
    ...
    val tempDir: Option[String] = try {
      ...
      GetCachedS3QueryPath(statement, threadName)
        .orElse(unloadDataToS3(statement, connWithQG, resultSchema, threadName))
    } finally {
      connWithQG.close()
    }
  ...
  private def readRDDFromParquet[T](resultSchema: StructType, filesToRead: Seq[String]): RDD[T] = {
    ...
    val conversionNeeded = resultSchema.fields.exists({ field: StructField =>
      field.dataType match {
        ...
        case TimestampType | TimestampNTZType | ShortType | ByteType => true
        case _ => false
      }
